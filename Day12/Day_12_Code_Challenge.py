...
print(" How many people in the given training set survived the disaster ? ",df['Survived'].value_counts()[1])
print(" How many people in the given training set died ?",df['Survived'].value_counts(normalize = True)[0])
print("survival rates as proportions ",df['Survived'].value_counts(normalize = True)[1])
df_survived=df.groupby(['Survived','Sex'])
print("Males that survived vs males that passed away ",len(df_survived.groups[(1,'female')])/len(df_survived.groups[(0,'female')]))
print("Females that survived vs Females that passed away ",len(df_survived.groups[(1,'female')])/len(df_survived.groups[(0,'female')]))
df["Child"] = df["Age"].map(lambda x: 1 if x < 18 else 0 )
df_child_survived=df.groupby(['Survived','Child'])
print("Children Alive :",len(df_child_survived.groups[(1,1)]))
print("Children Dead :",len(df_child_survived.groups[(0,1)]))
...
print(" Minimum Price : {}, Maximum Price: {}, Average Price: {} and Standard Deviation :{} ".format(data.max(),data.min(),data.mean(),data.std()))
...
print("1: count of Churned customer availing both voice mail plan and international plan: ",len(df.loc[(df['churn'] == True) & (df['voice mail plan'] == 'yes') & (df['international plan'] == 'yes')]))
#df[df['total intl calls'].isnull()]
#df[df['total intl charge'].isnull()]
df['intl price']=df['total intl minutes']*df['total intl charge']
df_charge=df.groupby(['churn'])
print("Total charges for International call by churned",)